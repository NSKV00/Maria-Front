...
const pegarVendasPorData = async (opcao: "hoje" | "ontem" | "anteontem",offset2) => {
  const hoje = new Date();
  let dataFiltro = hoje;
  ...
  setVenda(vendasConvertidas.sort((a: { id: number }, b: { id: number }) => a.id - b.id));
    ...
    useEffect(()=>{
            ...
            if (!token){
                ...
                validateUser(token)
                pegarTodasVendas(undefined, undefined, offset)
            }
        },[])    
    ...
    return<>
        ...
        <main>
           ...
           await pegarTodasVendas(valor,undefined,0)
            }
        }/>
                <select className={style.selectEstilizado} onChange={(e) => {
                    ...
                    if (valor == "todos") {
                        ...
                        setFiltroData(valor)
                        pegarVendasPorData(e.target.value as "hoje" | "ontem" | "anteontem",0)
                    }
...
<button disabled={offset <= 0} onClick={async () => {
    ...
    const novoOffset = prevOffset - 8;
        if (filtroData) {
            pegarVendasPorData(filtroData, novoOffset)
        } else {
            pegarTodasVendas(undefined, undefined, novoOffset)
        }
        return novoOffset;
    return novoOffset;});
    }}> 
    Prev</button>
<button disabled={venda.length < 8} onClick={async () => {
    setOffset(prevOffset => {
    const novoOffset = prevOffset + 8;
        if (filtroData) {
            pegarVendasPorData(filtroData, novoOffset)
    ...
    <div className={`${style.modalContent} ${style.divBotaoVenda}`} onClick={e => e.stopPropagation()}>
        ...
        <button onClick={async () => {
            const deletado = await deletarProduto(vendaSelecionada.id)
            if (deletado) {
